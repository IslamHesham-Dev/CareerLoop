...
class _ToneScreenState extends State<ToneScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<ToneRepository>().loadQuestions();
    });
  }

  @override
  Widget build(BuildContext context) {
    final repository = context.watch<ToneRepository>();
    final showOnboarding = _editing || !repository.hasProfile;
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
      ...
      body: SafeArea(
        top: false,
        child: repository.loadingQuestions && repository.questions.isEmpty
            ? const LensLoading(label: 'Bringing your onboarding questions into focus…')
            : repository.questions.isEmpty
                ? LensError(
                    message: repository.error ??
                        'The onboarding questions could not be loaded.',
                    onRetry: () => context.read<ToneRepository>().loadQuestions(),
                  )
                : AnimatedSwitcher(
...
class _ToneOnboarding extends StatefulWidget {
  final ToneRepository repository;
